"""Local validation of submission_medium.py in the REAL kaggle simulation.

Runs the exact submitted `agent(obs, config)` vs scripted baselines (and vs the greedy
bot, to confirm the anchor ordering medium >= greedy) and reports win rate, so a
broken/weak agent is caught before spending a daily submission.

    .venv\\Scripts\\python.exe scripts\\eval_medium.py --episodes 20
"""
...
from submission_medium import agent as medium_agent  # noqa: E402
...
def main():
    ...
    medium = FunctionAgent(medium_agent, name="medium")
    opponents = {
        "noop": NoopAgent(),
    ...
    for name, opp in opponents.items():
        w = [0, 0, 0]  # medium, opp, draw
        sample = []
        for ep in range(args.episodes):
            winner, scores = play_once(medium, opp, seed=1000 + ep)
            w[2 if winner == -1 else winner] += 1
            if ep < 3:
                sample.append(scores)
        n = args.episodes
        print(f"medium vs {name:8s} | win {w[0]/n:5.0%}  lose {w[1]/n:5.0%}  draw {w[2]/n:5.0%}"
